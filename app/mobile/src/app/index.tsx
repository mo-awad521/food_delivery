...
export default function HomeScreen(){
  ...
  return (
    <View style={styles.container}>
      ...
      {health && <View style={styles.statusBox}>
        <Text style={styles.statusText}>API Status:{health.status}</Text>
        <Text style={styles.timestampText}>{ new Date(health.timestamp).toLocaleString() }</Text>
        </View>}
        {error && <View style={styles.errorBox}>
          <Text style={styles.errorText}>Error occurred while fetching health data.</Text>
        </View>}
    </View>
  )
}
